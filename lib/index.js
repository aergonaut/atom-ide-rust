...
const RLS_BEGIN_BUILD = "rustDocument/beginBuild";
const RLS_DIAGNOSTICS_END = "rustDocument/diagnosticsEnd";
...
class RustLanguageClient extends AutoLanguageClient {
  ...
  preInitialization(connection) {
    connection.onCustom(RLS_BEGIN_BUILD, (payload) => {
      if (this._busySignalService != null) {
        if (this._lastBusyMessage) {
          this._lastBusyMessage.dispose();
        }

        const status = "RLS Analyzing";
        this._lastBusyMessage = this._busySignalService.reportBusy(status);
      }
    });

    connection.onCustom(RLS_DIAGNOSTICS_END, (payload) => {
      if (this._busySignalService != null) {
        if (this._lastBusyMessage) {
          this._lastBusyMessage.dispose();
        }
